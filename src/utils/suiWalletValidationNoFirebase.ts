...
export async function getUsdtBalance(userAddress: string): Promise<number> {
  try {
    ...
    // Convert from smallest unit to USDT (assuming 6 decimals)
    return parseFloat(balance.totalBalance) / 1e6;
  } catch (error) {
...
/**
 * Validate user wallet for OFF-RAMP transaction (NO FIREBASE)
 */
export async function validateUserWalletForOffRampNoFirebase(
  userAddress: string,
